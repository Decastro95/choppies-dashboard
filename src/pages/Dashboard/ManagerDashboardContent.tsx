  ...
  useEffect(() => {
    async function loadData() {
      ...
      const { data: sumData } = await supabase
        ...
        .select("*")
        .maybeSingle();
      const { data: lowData } = await supabase.from<LowStock>("low_stock_view").select("*");
      const { data: expData } = await supabase.from<ExpiringGoods>("expiring_goods_view").select("*");
            ...
            <div className="bg-white rounded-2xl p-6 shadow">
              ...
              <ul>
                {lowStock.map((item, idx) => (
                  <li key={idx}>{item.product_name} — Qty: {item.quantity}</li>
                ))}
              </ul>
            </div>
            <div className="bg-white rounded-2xl p-6 shadow">
              ...
              <ul>
                {expiring.map((item, idx) => (
                  <li key={idx}>{item.product_name} — {item.days_to_expiry} days left</li>
                ))}
              </ul>
            </div>
          </div>
